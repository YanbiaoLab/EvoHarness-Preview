...
# Layering: evocore must not import evoguard, so the local L0 pre-gate is an
# injected callable (wired by recipes/tasks); stdlib-only HTTP via urllib,
# with the transport injectable like LLMClient/TaskBundle for offline tests.
...
class RemoteGrader:
    ...
    markers / antihack static scan). Wire evoguard here from the recipe layer.
