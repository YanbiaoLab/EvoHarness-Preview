...
class Grade:
    ...
    n_units: int = 0                          # attempted units, 0 = not reported
    trustworthy_units: int | None = None      # usable evidence units; None = legacy
    sem: float = 0.0                          # standard error of fitness
    # 结构化故障词表(evaluation/faults.py),None = 未上报;wire v1 可选键。
...
def coerce_grade(raw: GradeValue) -> dict:
    ...
    if isinstance(grade.fitness, bool) or not isinstance(
        grade.fitness, (int, float)
    ):
    ...
    if grade.fault_kind is not None and (
        not isinstance(grade.fault_kind, str) or not grade.fault_kind.strip()
    ):
        raise TypeError("fault_kind must be a non-empty string or None")
    if grade.passed and grade.fault_kind is not None:
        raise ValueError("passed grade cannot carry fault_kind")
    for name in ("visible_metrics", "hidden_metrics"):
        if not isinstance(getattr(grade, name), dict):
            raise TypeError(f"{name} must be a dict")
    if grade.structured_feedback is not None and not isinstance(
        grade.structured_feedback, dict
    ):
        raise TypeError("structured_feedback must be a dict or None")
    for name in ("fault", "artifacts_ref"):
        value = getattr(grade, name)
        if value is not None and not isinstance(value, str):
            raise TypeError(f"{name} must be a string or None")
    for name in ("notes", "stdout_log", "stderr_log"):
        if not isinstance(getattr(grade, name), str):
            raise TypeError(f"{name} must be a string")
    if (
        isinstance(grade.stage_reached, bool)
        or not isinstance(grade.stage_reached, int)
        or grade.stage_reached < 0
    ):
        raise ValueError("stage_reached must be a non-negative integer")
    if (
        isinstance(grade.n_units, bool)
        or not isinstance(grade.n_units, int)
        or grade.n_units < 0
    ):
        raise ValueError("n_units must be a non-negative integer")
    if grade.trustworthy_units is not None and (
        isinstance(grade.trustworthy_units, bool)
        or not isinstance(grade.trustworthy_units, int)
        or grade.trustworthy_units < 0
        or grade.trustworthy_units > grade.n_units
    ):
        raise ValueError(
            "trustworthy_units must be None or an integer in [0, n_units]"
        )
    for name in ("sem", "execution_time", "eval_cost_usd"):
        value = getattr(grade, name)
        if (
    ...
    return {
        ...
        "trustworthy_units": grade.trustworthy_units,
        "sem": float(grade.sem),
        "fault_kind": grade.fault_kind,
    }
