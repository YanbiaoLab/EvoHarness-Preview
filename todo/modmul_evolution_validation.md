...
> ✅ **判决已出(2026-08-04 标注):进化在 modmul 上有效,且证据远超本文
> 预设的判据。**搜索独立完成四次"发现→兑现"闭环:张量核推理(tier 10 时钟
> 2.6×)、整步 CUDA graphs(再 2×,且凭经验台账跨重启自主复活)、去偏置
> 输出头(消随机权重蒙分通道)、混合精度漂移修复(t10 精度 0.97→0.99)。
> 认证成品 17b8eb34 即 r15 第 23 代候选原样打包;其血统在种子适应度之下
> 潜行 20+ 代后爆发 —— "低分祖先产出高分后代"的实例。反面同样记录在案:
> 搜索七代没跨过的最后一题,权重汤 20 分钟跨过(种群互补性,框架已补
> merge 算子)。全景见 experiments/modmul/实验报告_SAIR模乘挑战_2026-08-03.md。
