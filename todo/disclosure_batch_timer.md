...
> **2026-08-04 状态**:仍待负责人决定。补充一个减压事实:最终认证成品
> **不依赖**该计时器语义(总钟 250.7s,留有约 49s 余量),发与不发都不
> 影响成绩有效性;窗口在 2026-08-12 提交截止前。
...
诚实地把 tiers 1–10 塞进 300 秒。数字随配置演进,按测量日期列:

```
2026-07-27(MAX_WIDTH=4096 时代,草稿正文引用的那组):
  tier 10 单层 788 秒 / 100 题,tiers 0–9 累计 ≈ 190 秒,总计 ≈ 978 秒

2026-07-29(r12 第 0 代,MAX_WIDTH=2048、三遍 pass、精确位长分组):
  tiers 0–9 累计 469.8 秒,tier 10 未跑;官方口径 h90=8,差 1.57 倍

2026-07-29 起飞中(两遍 pass + Δ 分层 + 分桶批处理 + k=2 候选):
  三个乘性杠杆,目标把总钟压进 300 秒 —— h90=10 不再依赖计时器行为
```

草稿正文引用的 788/190/978 是真实测量,作为机制演示仍然成立;
若发送时想给最新数字,换成 r12 那组即可。

**发送前多核对一件事:**榜上现挂的那个 h90=2 条目是否依赖本披露描述的
批间计时行为。若不依赖(它的问题是预处理约减,与计时无关),正文里
"We have not submitted anything relying on this" 成立;若有任何依赖,
先撤或先改那个条目再发。
